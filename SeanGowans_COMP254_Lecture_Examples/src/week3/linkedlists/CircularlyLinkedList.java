...
public class CircularlyLinkedList<E> implements Cloneable {
  ...
  public E removeFirst() {                   // removes and returns the first element
    ...
    public CircularlyLinkedList<E> clone() throws CloneNotSupportedException {
        ...
        // clone() must return a new object, even when the list is empty.
        if(this.size == 0) {
            other.tail = null;
            other.size = 0;
            return other;
        ...
        Node<E> headOriginal = this.tail.getNext();

        // copy the first node (new head) (Next Node will be Null)
        Node<E> headCopy = new Node<>(headOriginal.getElement(), null);
        Node<E> copyTail = headCopy;

        // walk the rest of the original circle until we're back at headOriginal
        Node<E> walk = headOriginal.getNext();
        while(walk != headOriginal) {
            Node<E> newest = new Node<>(walk.getElement(), null);
            copyTail.setNext(newest);
            copyTail = newest;
            walk = walk.getNext();
        }

        // close the circle in the copy
        copyTail.setNext(headCopy);

        // set tail + size in the clone
        other.tail = copyTail;
